...
uint32_t Adafruit_Fingerprint::upImage(void) //CREATED 29 March 2016
{
  uint8_t packet[] = {FINGERPRINT_UPIMG};
  writePacket(theAddress, FINGERPRINT_COMMANDPACKET, 3, packet);
...
//transfer a fingerprint template to Char Buffer 1 from host computer
uint8_t Adafruit_Fingerprint::downBuffer(void) {
    uint8_t packet[] = {FINGERPRINT_DOWNLOAD, 0x01}; // 0x01 for charbuffer 1
...
uint8_t Adafruit_Fingerprint::createFollowUp(uint8_t buffer[128]){
	uint8_t packet[128];
	for (int i=0;i<128;i++) packet[i] = buffer[i];	
    writePacket(theAddress, 0x02,  130, packet);
	    uint8_t len = getReply(packet);
...
uint8_t Adafruit_Fingerprint::endFollowUp(uint8_t buffer[128]){
	uint8_t packet[128]; 
	for (int i=0;i<128;i++) packet[i] = buffer[i];	
    writePacket(theAddress, 0x08,  sizeof(packet)+2, packet);
	    uint8_t len = getReply(packet);
...
uint8_t Adafruit_Fingerprint::storeModel(uint16_t id) {
  uint8_t packet[] = {FINGERPRINT_STORE, 0x01, id >> 8, id & 0x00FF};
  writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(packet)+2, packet);
  uint8_t len = getReply(packet);
...
uint8_t Adafruit_Fingerprint::loadModel(uint16_t id) {
    uint8_t packet[] = {FINGERPRINT_LOAD, 0x01, id >> 8, id & 0x00FF};
    writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(packet)+2, packet);
    uint8_t len = getReply(packet);
...
uint8_t Adafruit_Fingerprint::fingerFastSearch(void) {
  ...
  uint8_t packet[] = {FINGERPRINT_HISPEEDSEARCH, 0x01, 0x00, 0x00, 0xFF, 0xFF};
  writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(packet)+2, packet);
  uint8_t len = getReply(packet);
...
uint16_t Adafruit_Fingerprint::getTemplateCount(void) {
